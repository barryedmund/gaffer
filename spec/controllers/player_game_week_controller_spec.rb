	...
	describe "POST create" do
		describe "with valid params" do
  			it "creates a new PlayerGameWeek" do
    			expect { PlayerGameWeek.create(player_id: player.id, game_week_id: current_game_week.id, minutes_played: 90, goals: 1, clean_sheet: true) }.to change{ PlayerGameWeek.count }.by(1)
  			end

  			it "assigns a newly created player_game_week as @player_game_week" do
  				player_game_week = PlayerGameWeek.create(player_id: player.id, game_week_id: current_game_week.id, minutes_played: 90, goals: 1, clean_sheet: true)
